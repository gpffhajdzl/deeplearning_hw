...
def generate(number=100, seed=None, plot=True, num_class=3, sigma=1.0):
    ...
    cov_dict = {0: [[0.25, -0.1], [-0.1, 0.15]], 1: [[0.2, 0], [0, 0.25]], 2: [[0.25, -0.15], [-0.15, 0.4]],
                3: [[0.4, 0], [0, 0.4]]}
    ...
    for i in range(num_class):
        x_data = np.random.multivariate_normal(mean_dict[i], np.array(cov_dict[i]) * sigma2,
                                               class_number).T + np.random.uniform(-0.3, 0.3, class_number)
